  ...
  def create
    ...
    ActiveRecord::Base.transaction do
      images.each do |image|
        ...
        errors.push("「#{@media[:name]}.#{@media[:extension]}」 #{@media.errors.full_messages.join(',')}") if !@media.save
      end
  ...
  def update
    ...
    ActiveRecord::Base.transaction do
      ...
      if image
        ...
        image.tempfile = ImageProcessing::MiniMagick.source(image.tempfile).convert(@media.extension).call
        @media.remove_image!
      end
